...
struct latch{
	...
	void update(int des,int reg1,int reg2){
		d = des;
		r1 = reg1;
		r2 = reg2;
	}
	...
	void print(){
		cout<<"des: "<<d<<" source1: "<<r1<<" source2: "<<r2<<" stall: "<<stall<<endl;
	}
...
struct MIPS_Architecture
{
	...
	// whether a reg is getting dependant on that value
	...
	{
		l[2]->update(l[1]->r2+l[1]->r1,l[1]->r1,l[1]->r2);
		// l[1]->clear();
		return 0;
	}
	...
	{
		l[2]->update(l[1]->r1 - l[1]->r2,l[1]->r1,l[1]->r2);
		// l[1]->clear();
		return 0;
	}
	...
	{
		if(l[1]->r1==l[1]->r2){
			l[2]->update(1,l[1]->r1,l[1]->r2);
		}else{
			l[2]->update(0,l[1]->r1,l[1]->r2);
		}
		// p->clear();
		return 0;
	}
	...
	int sw(std::string r, std::string location, std::string unused1 = "")
	{
		...
		if (address < 0)
			return abs(address);
		//cout<<l[2]->r1<<" "<<l[2]->r2<<endl;
		//cout<<"Memory write happened at "+to_string(address*4)+" = "+to_string(l[2]->r1)<<endl;
		data[address] = l[2]->r1;
		mem.push_back(address*4);
		mem.push_back(l[2]->r1);
		//PCnext = PCcurr + 1;
		return 0;
	}
		...
		while(!completed() || PCnext<commands.size()){
			...
			if(pip[4]>=0){
				if(commands[pip[4]][0]=="add" || commands[pip[4]][0]=="addi" || commands[pip[4]][0]=="sub" || commands[pip[4]][0]=="mul" || commands[pip[4]][0]== "slt" || commands[pip[4]][0]=="lw"  ){
					...
					registers[registerMap[commands[pip[4]][1]]]=l[3]->d;
					if(pip[2]>=0 && getReg(commands[pip[4]][1])==getReg(commands[pip[2]][2])  ){
						if(commands[pip[2]][0]=="sw" || commands[pip[2]][0]=="beq" || commands[pip[2]][0]=="bne"){
							//cout<<"forwarding1 from MEM to ALU"<<endl;
							l[1]->r2 = l[3]->d;
						}else{
							//cout<<"forwarding2 from MEM to ALU"<<endl;
							l[1]->r1 = l[3]->d;
						}
					}	
					if(pip[2]>=0 && getReg(commands[pip[4]][1])==getReg(commands[pip[2]][3])  ){
						//cout<<"forwarding3 from MEM to ALU"<<endl;
						l[1]->r2 = l[3]->d;
					}
					if(pip[2]>=0 && getReg(commands[pip[4]][1])==getReg(commands[pip[2]][1] ) &&  (commands[pip[2]][0]=="sw" || commands[pip[2]][0]=="beq" || commands[pip[2]][0]=="bne")){
						//cout<<"forwarding4 from MEM to ALU"<<endl;
						l[1]->r1 = l[3]->d;
					}

				}
				if(commands[pip[4]][0]=="lw" && pip[3]>0 && commands[pip[3]][0]=="lw " && getReg(commands[pip[4]][1])==getReg(commands[pip[3]][2])){
					l[2]->r1 = l[3]->d;
					//cout<<"forwarding1 from MEM to MEM"<<endl;
				}
				if(commands[pip[4]][0]=="lw" && pip[3]>0 && commands[pip[3]][0]=="sw " && getReg(commands[pip[4]][1])==getReg(commands[pip[3]][2])){
					l[2]->r2 = l[3]->d;
					//cout<<"forwarding2 from MEM to MEM"<<endl;
				}
				if(commands[pip[4]][0]=="lw" && pip[3]>0 && commands[pip[3]][0]=="sw " && getReg(commands[pip[4]][1])==getReg(commands[pip[3]][1])){
					l[2]->r1 = l[3]->d;
					//cout<<"forwarding3 from MEM to MEM"<<endl;
				}
			...
			if(pip[3]>=0){
				...
				}else if(commands[pip[3]][0]=="lw"){
					exit_code ret = (exit_code) instructions[commands[pip[3]][0]](*this, commands[pip[3]][1], commands[pip[3]][2], commands[pip[3]][3]);
					// if(pip[2]>=0 && getReg(commands[pip[3]][1])==getReg(commands[pip[2]][2])  ){
					// 	if((commands[pip[2]][0]=="sw" || commands[pip[2]][0]=="lw")){
					// 		cout<<"forwarding from MEM to MEM"<<endl;
					// 		l[1]->r1 = l[3]->d;
					// 	}else{
					// 		cout<<"forwarding from MEM to ALU"<<endl;
					// 		l[1]->r1 = l[3]->d;
					// 	}
						
					// }
					// if(pip[2]>=0 && getReg(commands[pip[3]][1])==getReg(commands[pip[2]][3])  ){
					// 	cout<<"forwarding from MEM to ALU"<<endl;
					// 	l[1]->r2 = l[3]->d;
						
					// }
					
					// if(pip[2]>=0 && getReg(commands[pip[3]][1])==getReg(commands[pip[2]][1]) && commands[pip[2]][0]=="sw" ){
					// 	l[2]->r1 = l[3]->d;
					// 	cout<<"forwarding from MEM to ALU"<<endl;
					// }
				}else{
					l[3]->update(l[2]->d,l[2]->r1,l[2]->r2);
					if(pip[2]>=0 && getReg(commands[pip[3]][1])==getReg(commands[pip[2]][2]) ){
						if(commands[pip[2]][0]=="sw" || commands[pip[2]][0]=="beq" || commands[pip[2]][0]=="bne"){
							//cout<<"forwarding from ALU1 to ALU"<<endl;
							l[1]->r2 = l[3]->d;
						}else{
							//cout<<"forwarding from ALU2 to ALU"<<endl;
							l[1]->r1 = l[3]->d;
						}
					}
					if(pip[2]>=0 && getReg(commands[pip[3]][1])==getReg(commands[pip[2]][3]) ){
						l[1]->r2 = l[3]->d;
						//cout<<"forwarding ALU3 to ALU"<<endl;
					}
					if(pip[2]>=0 && getReg(commands[pip[3]][1])==getReg(commands[pip[2]][1]) && (commands[pip[2]][0]=="sw" || commands[pip[2]][0]=="beq" || commands[pip[2]][0]=="bne") ){
						l[1]->r1 = l[3]->d;
						//cout<<"forwarding ALU4 to ALU"<<endl;
					}
				}
				//ful[4]=1;	
			...
			if(pip[2]>=0){
				...
				if(l[1]->stall==1){
					...
					if(commands[pip[2]][0]=="beq" || commands[pip[2]][0]=="bne" ){
						...
						pip[2]=-1;
						pip[3]=pip[2];
					}else if(commands[pip[2]][0]== "add" || commands[pip[2]][0]== "sub" || commands[pip[2]][0]== "mul"  || commands[pip[2]][0]== "slt" || commands[pip[2]][0]== "addi" ){
						exit_code ret = (exit_code) instructions[commands[pip[2]][0]](*this, commands[pip[2]][1], commands[pip[2]][2], commands[pip[2]][3]);
						pip[3]=pip[2];
						pip[2]=-1;
					}else{
						l[2]->update(l[1]->d,l[1]->r1,l[1]->r2);
						pip[3]=pip[2];
						pip[2]=-1;
					}
				}
			...
			if(pip[1]>=0 && pip[2]==-1 ){
				...
				if( commands[pip[1]][0]== "add" || commands[pip[1]][0]=="sub" || commands[pip[1]][0]=="mul" || commands[pip[1]][0]=="slt"  ){
					l[1]->r1 = registers[registerMap[commands[pip[1]][2]]];
					l[1]->r2 = registers[registerMap[commands[pip[1]][3]]];
				} else if( commands[pip[1]][0]=="beq" || commands[pip[1]][0]=="bne" ){
					l[1]->r1 = registers[registerMap[commands[pip[1]][1]]];
					l[1]->r2 = registers[registerMap[commands[pip[1]][2]]];
				}else if(commands[pip[1]][0]=="sw" ){
					l[1]->r1 = registers[registerMap[commands[pip[1]][1]]];
					l[1]->r2 = registers[registerMap[getReg(commands[pip[1]][2])]];
				}else if(commands[pip[1]][0]=="lw"){
					l[1]->r1 = registers[registerMap[getReg(commands[pip[1]][2])]];
				}else if(commands[pip[1]][0]=="addi" ){
					l[1]->r1 = registers[registerMap[commands[pip[1]][2]]];
					l[1]->r2 = stoi(commands[pip[1]][3]);
					...
					//cout<<"jumping to "+commands[pip[1]][1]<<endl;
					pip[0]=-1;
					pip[1]=-1;
				}
				if(pip[3]>=0 && pip[1]>=0 && (getReg(commands[pip[1]][2])==getReg(commands[pip[3]][1]) || getReg(commands[pip[1]][3])==getReg(commands[pip[3]][1])) && commands[pip[3]][0]=="lw"){
					l[1]->stall = 1;
				}
				pip[2]=pip[1];
				pip[1]=-1;
				
			}else if(pip[1]>=0 && pip[2]!=-1){
				++commandCount[pip[1]];
			}
